...
export class OverflowedText extends PureComponent<Props> {
  ...
  render() {
    ...
    let textStyle: Record<string, string> = { maxWidth: getPixel(maxWidth), ...style };
    ...
    return <div className={cls} style={textStyle} ref={this.init}>
      {this.state.isOverflow ? (
        <ConfigProvider prefixCls="antd5">
          <Tooltip title={text} placement="topLeft" overlayStyle={{ maxWidth: '50vw' }} {...props}>
            {text}
          </Tooltip>
        </ConfigProvider>
      ) : (
